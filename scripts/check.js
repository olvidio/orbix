...
function fnjs_def_tabla(tabla) {
    ...
    url = "1/src/usuarios/preferencias_guardar";
    parametros = 'que=slickGrid&tabla=' + tabla + '&sPrefs=' + sPrefs + '1';
    $.ajax({
        url: url,
        type: 'post',
        data: parametros,
...
function fnjs_link_menu(id_grupmenu) {
    var parametros = 'id_grupmenu=' + id_grupmenu + '&PHPSESSID=1';
...
function fnjs_link_submenu(url, parametros
    ...
    if (url === "1/programas/pag_html_editar.php") {
        window.open(url + '?' + parametros);
    } else {
        fnjs_guardar_estado();
        $('#main').attr('refe', url);
...
function fnjs_ref_absoluta(base, path
    ...
    a = 0;
    if (base.match(/^1/)) {	// si es un directorio
        base = base.replace('1', '');
        inicio = protocol + '1';
        a = 2;
    } else {
...
function fnjs_def_tabla(tabla) {
    ...
    sPrefs = JSON.stringify(oPrefs);
    url = "1/src/usuarios/infrastructure/ui/http/controllers/preferencias_guardar.php";
    parametros = 'que=slickGrid&tabla=' + tabla + '&sPrefs=' + sPrefs + '1';
    $.ajax({
        url: url,
        type: 'post',
