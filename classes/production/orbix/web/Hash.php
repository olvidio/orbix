        ...
        if (isset($aPOST['h'])) {
            ...
            if ($salta === 1) {
                if (ConfigGlobal::is_debug_mode()) {
                    ...
                    $err = _("problema general de seguridad") . "\n";
                    $err .= _("para ver más detalles activa el mode debug en core\configGlobal");
                    //$_SESSION['oGestorErrores']->addErrorSec($err, $salta_txt, __LINE__, __FILE__);
...
//					session_start();
            ...
            if ($salta === 0 && (ConfigGlobal::mi_id_role() === 1)) {
                if (!str_contains($_SERVER['SCRIPT_NAME'], '2fa.php')) {
    ...
    static function cmdConParametros(string $sUrl_full)
    {
        ...
        $sUrl_int = str_replace(ConfigGlobal::$web_port_sf, ConfigGlobal::$web_port, $sUrl_org);
        return $sUrl_int;
    }
    ...
    {
        $sUrl_int = str_replace(ConfigGlobal::$web_port_sf, ConfigGlobal::$web_port, $sUrl_full);
        return $sUrl_int;
    }
    ...
    private function realFullUrl()
    {
        ...
        */
        if ($_SERVER['SERVER_ADDR'] === $_SERVER['SERVER_NAME']) {
            ...
            $sUrl = $_SERVER["REQUEST_SCHEME"] . '://' . $_SERVER["SERVER_NAME"] . $port . $_SERVER["SCRIPT_NAME"];
        } else {
            if (str_contains($_SERVER['HTTP_HOST'], ':')) {
                $sUrl = $_SERVER["REQUEST_SCHEME"] . '://' . $_SERVER["HTTP_HOST"] . $_SERVER["SCRIPT_NAME"];
            } else {
                $sUrl = $_SERVER["REQUEST_SCHEME"] . '://' . $_SERVER["SERVER_NAME"] . ':' . $_SERVER["SERVER_PORT"] . $_SERVER["SCRIPT_NAME"];
            }
        }
        return $sUrl;
    }
