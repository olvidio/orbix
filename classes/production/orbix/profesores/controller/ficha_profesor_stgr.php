...
use src\profesores\infrastructure\factories\ProfesorDirectorServiceFactory;
use src\profesores\infrastructure\factories\ProfesorDocenciaStgrServiceFactory;
use src\profesores\infrastructure\factories\ProfesorAmpliacionServiceFactory;
use src\profesores\infrastructure\factories\ProfesorPublicacionServiceFactory;
use src\profesores\infrastructure\factories\ProfesorTituloEstServiceFactory;
use src\profesores\infrastructure\factories\ProfesorJuramentoServiceFactory;
use src\profesores\infrastructure\factories\ProfesorTipoServiceFactory;
use src\profesores\infrastructure\factories\ProfesorLatinServiceFactory;
use src\profesores\infrastructure\factories\ProfesorServiceFactory;
...
$profesorLatinService = ProfesorLatinServiceFactory::create();
$oProfesorLatin = $profesorLatinService->findById($id_nom);
...
$profesorService = ProfesorServiceFactory::create();

$cProfesores = $profesorService->getProfesores($aWhere, $aOperador);
$a_nombramientos = [];
$dep = '';
foreach ($cProfesores as $oProfesor) {
    ...
    $profesorTipoService = ProfesorTipoServiceFactory::create();
    $tipo_profesor = $profesorTipoService->findById((int)$id_tipo_profesor)->getTipo_profesor();
...
if (empty($Qprint)) { // si no es para imprimir muestro todos los datos
    ...
    $profesorDirectorService = ProfesorDirectorServiceFactory::create();
    $cDirectores = $profesorDirectorService->getProfesoresDirectores($aWhere, $aOperador);
    $a_director = [];
    ...
    // juramento //////////////////////////
    $profesorJuramentoService = ProfesorJuramentoServiceFactory::create();
    $oJuramento = $profesorJuramentoService->findById($id_nom);
    ...
    $profesorPublicacionService = ProfesorPublicacionServiceFactory::create();
    $cProfesorPublicaciones = $profesorPublicacionService->getProfesorPublicaciones(array('id_nom' => $id_nom, '_ordre' => 'f_publicacion'));
    $a_publicaciones = [];
...
$profesorTituloEstService = ProfesorTituloEstServiceFactory::create();
$cTitulosEst = $profesorTituloEstService->getProfesorTitulosEst(array('id_nom' => $id_nom, '_ordre' => 'year'));
$a_curriculum = [];
...
// Using the service created earlier
$cProfesores = $profesorService->getProfesores($aWhere, $aOperador);
$a_nombramientos = [];
$id_departamento = '';
foreach ($cProfesores as $oProfesor) {
...
$profesorAmpliacionService = ProfesorAmpliacionServiceFactory::create();
$cProfesorAmpliaciones = $profesorAmpliacionService->getProfesorAmpliaciones($aWhere, $aOperador);
$a_ampliacion = [];
$id_departamento = '';
...
$cProfesorCongresos = $profesorCongresoService->getProfesorCongresos(array('id_nom' => $id_nom, '_ordre' => 'f_ini'));
...
$profesorDocenciaService = ProfesorDocenciaStgrServiceFactory::create();
$cDocencias = $profesorDocenciaService->getProfesorDocenciasStgr(array('id_nom' => $id_nom, '_ordre' => 'curso_inicio,id_asignatura'));
