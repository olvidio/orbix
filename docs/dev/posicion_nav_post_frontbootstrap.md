...
**Audit:** `php tools/audit/audit_posicion_nav_migration.php --strict`  
...
| Comando | Uso |
|---------|-----|
| `php tools/audit/audit_posicion_nav_migration.php` | Hallazgos por categoría |
| `php tools/audit/audit_posicion_nav_migration.php --only=sin_clear_stack` | Sin `ListNavSupport::bootRecordar` / `clear_inherited` |
| `php tools/audit/audit_posicion_nav_migration.php --only=padre_antes_recordar` | `setParametros(...,1)` antes de `recordar()` |
| `php tools/fix/fix_recordar_sin_post_limpio.php --apply` | Inserta `ListNavSupport::persistRecordarEntry` (ya aplicado en bloque) |
| `php tools/fix/fix_stack_before_recordar_v3.php --apply` | Reordena `goStack` antes de `recordar()` |
