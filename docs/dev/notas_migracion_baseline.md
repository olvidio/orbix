...
```
docs/legacy/obix/menus.csv → 7 entradas a apps/notas/controller/...
...
- Actualizar `docs/legacy/obix/menus.csv` y `proves/aux_metamenus.csv` a `frontend/notas/controller/...`.
...
- **Slice 13 completado**: `apps/notas/model/Tesera.php` (325 LOC) migrado a `src/notas/application/Tesera.php`. Mejoras respecto al legacy:
    ...
    - Magic numbers encapsulados como constantes: `ID_NIVEL_ASIG_DESDE/HASTA`, `ID_NIVEL_MAX_CUADRIENIO` (`2434`), `ID_ASIG_OPCIONAL_UMBRAL` (`3000`), `ID_ASIG_OPCIONAL_MAX` (`9000`), `ID_ASIG_FIN_CUADRIENIO` (`9998`), `PLAN_NUEVO` (`2026`), `PLAN_VIEJO` (`1997`), `ID_NIVEL_PLAN_1997_DESAPARECIDO` (`2114`), `ID_NIVEL_PLAN_1997_SUSTITUTOS` (`2112,2113`), `FECHA_LIMITE_PLAN_2026` (`2026-03-30`). Los listados de asignaturas filtran por `plan_estudios` (`IN_ARRAY`).
