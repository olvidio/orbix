...
- `tools/qa/check_vo_db_constraints.php`
- `tools/qa/vo_validation_registry.php` (registry de módulos/tablas/entidades)
- `tools/qa/read_vo_validation_errors.py` (lector compacto de errores)
...
```bash
php tools/qa/check_vo_db_constraints.php \
  --module=profesores \
  --esquema=H-dlbv \
  --limit=500 \
  --from-id=0 \
...
php tools/qa/check_vo_db_constraints.php \
  ...
  --max-batches=0 \
  --format=json \
  --output=docs/dev/reports/vo_db_validation_report.json
```
...
```bash
python3 tools/qa/read_vo_validation_errors.py \
  --input docs/dev/reports/vo_db_validation_report.json \
  --dedupe
```
...
php tools/qa/check_vo_db_constraints.php \
  ...
  --max-batches=0 \
  --preflight-missing=docs/dev/reports/vo_missing_tables.json \
  --format=json \
  --output=docs/dev/reports/vo_db_validation_report.json
```

El fichero `docs/dev/reports/vo_missing_tables.json` contendrá `schema`, `module`, `table` y `entity`.
...
- `--module=all` ejecuta todos los módulos definidos en `tools/qa/vo_validation_registry.php`.
