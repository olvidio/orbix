---
tipo: "ayuda_ia"
subtipo: "flujo"
modulo: "certificados"
titulo: "Certificado Emitido Lista"
flujo: "certificados.certificado_emitido_lista.gestionar.flujo"
preguntas: ["Como obtener datos en Certificado Emitido Lista?"]
pantallas_principales: []
fragmentos: ["certificados.pantalla.certificado_emitido_lista"]
endpoints: ["/src/certificados/certificado_emitido_lista_datos"]
source: "docs/catalogo/certificados/flujos/certificado_emitido_lista.md"
estado_revision: "generado"
---

# Ayuda IA - Certificado Emitido Lista

Usa este documento para responder preguntas de usuario sobre como trabajar con `Certificado Emitido Lista`.

## Cuando Usar Esta Ayuda

Responder con esta ayuda cuando el usuario pregunte por:
- Como obtener datos en Certificado Emitido Lista?

## Donde Entrar

- Pantalla pendiente de revisar.

## Como Responder

Da pasos cortos y orientados a usuario. Si falta ruta de menu, dilo como pendiente de documentar.

## Obtener datos

1. Revisar manualmente los pasos de esta accion.

Referencias tecnicas para verificar la respuesta:
- Ninguna referencia API inferida.

## Pantallas Y Fragmentos Relacionados

- `certificados.pantalla.certificado_emitido_lista`

## Objetivo

Consultar y gestionar certificados emitidos pendientes de envío en la región STGR.

## Limites De La Respuesta

- No inventar permisos si no estan documentados.
- No inventar rutas de menu si aparecen como pendientes.
- Si el usuario pregunta por errores concretos, responder que estan pendientes salvo que el catalogo los documente.
