...
respuesta_data_schema: "pasarela_NombreListaData"
respuesta_data: ["excepciones:list<array{id_tipo_activ: string, etiqueta: string, valor: string}>"]
...
- Payload en `data` (schema `pasarela_NombreListaData`):
  - `excepciones` (`list<array{id_tipo_activ: string, etiqueta: string, valor: string}>`)
