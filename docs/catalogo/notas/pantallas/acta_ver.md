...
- **Acta firmada (PDF):** cabecera en solo lectura; no alta de alumno. Si el fragmento está
  embebido en `acta_notas` y hay alumnos sin nota, se ofrece **nueva acta convocatoria** para
  calificar solo a esos alumnos (los del acta firmada no se tocan).
