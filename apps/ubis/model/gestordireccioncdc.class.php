...
class GestorDireccionCdc Extends GestorDireccion {
	/* ATRIBUTS ----------------------------------------------------------------- */
	...
	/**
	 * retorna l'array d'objectes de tipus DireccionCdc
	 *
	 * @param array aWhere associatiu amb els valors de les variables amb les quals farem la query
	 * @param array aOperators associatiu amb els valors dels operadors que cal aplicar a cada variable
	 * @return array Una col·lecció d'objectes de tipus DireccionCdc
	 */
	function getDirecciones($aWhere=array(),$aOperators=array()) {
		...
		if (($oDblSt = $oDbl->prepare($sQry)) === false) {
			$sClauError = 'GestorDireccionCdc.llistar.prepare';
			$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
			return false;
		}
		if (($oDblSt->execute($aWhere)) === false) {
			$sClauError = 'GestorDireccionCdc.llistar.execute';
			$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
			return false;
		}
		foreach ($oDblSt as $aDades) {
			$a_pkey = array('id_direccion' => $aDades['id_direccion']);
			$oRegion= new DireccionCdc($a_pkey);
