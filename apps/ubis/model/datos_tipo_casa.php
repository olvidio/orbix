...
switch ($padre) {
	case 'datos_sql':
		...
		if (empty($_POST['k_buscar'])) {
			...
			$aWhere=array('nombre_tipo_casa'=> $_POST['k_buscar']);
	...
	case 'datos_update':
		...
		if ($_POST['mod'] == 'editar' || $_POST['mod'] == 'eliminar') {
			if (!empty($a_pkey)) $oFicha = new TipoCasa($a_pkey);
		}
		if ($_POST['mod'] == 'nuevo') {
			$oFicha = new TipoCasa();
		}
...
$tit_buscar=_("buscar un tipo de casa");
$tit_txt=_("tipos de casa");
$explicacion_txt="";
?>
