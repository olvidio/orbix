...
if (!empty($Qid_menu) || !empty($Qnuevo)) {
    ...
    <form id="frm_menus" action="apps/devel/controller/menus_update.php">
        ...
        <table>
            <tr>
                <td><?= _("orden") ?></td>
                <td><input type="text" name="orden" value="<?= $row['orden'] ?>"></td>
            </tr>
            <tr>
                <td><?= _("texto menu") ?></td>
    ...
    <?php if (empty($Qnuevo)) { ?>
        <form id="frm_menus_2" action="apps/devel/controller/menus_update.php">
            ...
            <input type="button"
                   onclick="if (confirm('<?= addslashes(_("¿Está seguro?")) ?>')) { fnjs_enviar_formulario('#frm_menus_2','#ficha'); }"
                   value="<?= _("eliminar") ?>">
        </form>
        ...
        <form id="frm_menus_4" action="apps/devel/controller/menus_get.php">
            <input type="hidden" name="filtro_mod" value="<?= $Qfiltro_mod ?>">
            <input type="button" onclick="fnjs_enviar_formulario('#frm_menus_4','#ficha');"
                   value="<?= _("cancelar") ?>">
        </form>
        <?php
    }
