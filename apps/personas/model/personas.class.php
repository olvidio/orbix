 ...
 */
/**
 * Classe que implementa l'entitat p_supernumerarios
 *
 * @package delegación
 * @subpackage model
 * @author Daniel Serrabou
...
class PersonaS Extends PersonaDl {
		...
		if ($bInsert === false) {
			...
			if (($qRs = $oDbl->prepare("UPDATE $nom_tabla SET $update WHERE id_nom='$this->iid_nom'")) === false) {
				...
				if ($qRs->execute($aDades) === false) {
					$sClauError = 'PersonaS.update.execute';
					$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
					return false;
				}
			}
			...
			$campos="(id_nom,id_cr,id_tabla,dl,sacd,trato,nom,nx1,apellido1,nx2,apellido2,f_nacimiento,lengua,situacion,f_situacion,apel_fam,inc,f_inc,stgr,edad,profesion,eap,observ,id_ctr)";
			$valores="(:id_nom,:id_cr,:id_tabla,:dl,:sacd,:trato,:nom,:nx1,:apellido1,:nx2,:apellido2,:f_nacimiento,:lengua,:situacion,:f_situacion,:apel_fam,:inc,:f_inc,:stgr,:edad,:profesion,:eap,:observ,:id_ctr)";		
			if (($qRs = $oDbl->prepare("INSERT INTO $nom_tabla $campos VALUES $valores")) === false) {
				...
				if ($qRs->execute($aDades) === false) {
					$sClauError = 'PersonaS.insertar.execute';
					$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
					return false;
				}
			}
	...
	function getDatosCampos() {
		...
		$oPersonaSSet->add($this->getDatosDl());
		$oPersonaSSet->add($this->getDatosSacd());
		$oPersonaSSet->add($this->getDatosTrato());
		$oPersonaSSet->add($this->getDatosNom());
		$oPersonaSSet->add($this->getDatosNx1());
		$oPersonaSSet->add($this->getDatosApellido1());
		$oPersonaSSet->add($this->getDatosNx2());
		$oPersonaSSet->add($this->getDatosApellido2());
		$oPersonaSSet->add($this->getDatosF_nacimiento());
		$oPersonaSSet->add($this->getDatosLengua());
		$oPersonaSSet->add($this->getDatosSituacion());
		$oPersonaSSet->add($this->getDatosF_situacion());
		$oPersonaSSet->add($this->getDatosApel_fam());
