...
class GestorPersonaPub Extends core\ClaseGestor {
	/* ATRIBUTS ----------------------------------------------------------------- */
	...
	/**
	 * retorna l'array d'objectes de tipus PersonaPub
	 *
	...
	function getPersonasPubQuery($sQuery='') {
		...
		if (($oDblSt = $oDbl->query($sQuery)) === false) {
			$sClauError = 'GestorPersonaPub.query';
			$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
			return false;
		}
		foreach ($oDbl->query($sQuery) as $aDades) {
			$a_pkey = array('id_nom' => $aDades['id_nom']);
			$oPersonaPub= new PersonaPub($a_pkey);
			$oPersonaPub->setAllAtributes($aDades);
			$oPersonaPubSet->add($oPersonaPub);
		}
	 ...
	 * @return array Una col·lecció d'objectes de tipus PersonaPub
	 */
	function getPersonasPub($aWhere=array(),$aOperators=array()) {
		$oDbl = $this->getoDbl();
		...
		if (($oDblSt = $oDbl->prepare($sQry)) === false) {
			$sClauError = 'GestorPersonaPub.llistar.prepare';
			$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
			return false;
		}
		if (($oDblSt->execute($aWhere)) === false) {
			$sClauError = 'GestorPersonaPub.llistar.execute';
			$_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
			return false;
		}
