...
switch ($padre) {
	case 'datos_sql':
		...
		$oLista=new GestorTraslado();
		$Coleccion=$oLista->getTraslados(array('id_nom'=>$_POST['id_pau']));
		break;
	case 'datos_update':
		// para el update
		if ($_POST['mod'] == 'editar' || $_POST['mod'] == 'eliminar') {
			if (!empty($a_pkey)) $oFicha = new Traslado($a_pkey);
		}
		if ($_POST['mod'] == 'nuevo') {
			$oFicha = new Traslado(array('id_nom'=>$_POST['id_pau']));
		}
		break;
	case 'datos_form':
		// para el form
		if (!empty($a_pkey) && isset($_POST['mod']) && $_POST['mod'] == 'editar') {
			$oFicha = new Traslado($a_pkey);
		}
...
$tit_txt=_("dossier de traslados");
$explicacion_txt='';
?>
