...
/**
 * Clase que implementa la entidad resto(v|f).d_teleco_personas_ex
 *
 * @package delegación
 * @subpackage model
 ...
 * @created 01/10/2010
 */
class TelecoPersonaEx extends TelecoPersonaGlobal
{
    /* ATRIBUTOS ----------------------------------------------------------------- */
    ...
    public function DBGuardar()
    {
        ...
        $aDades['id_desc_teleco'] = $this->sdesc_teleco;
        ...
        if ($bInsert === false) {
            ...
            if (($oDblSt = $oDbl->prepare("UPDATE $nom_tabla SET $update WHERE id_item=$this->iid_item")) === false) {
                $sClauError = 'TelecoPersonaEx.update.prepare';
                ...
                try {
                    ...
                    $sClauError = 'TelecoPersonaEx.update.execute';
            ...
            $campos = "(id_nom,tipo_teleco,id_desc_teleco,num_teleco,observ)";
            $valores = "(:id_nom,:tipo_teleco,:id_desc_teleco,:num_teleco,:observ)";
            if (($oDblSt = $oDbl->prepare("INSERT INTO $nom_tabla $campos VALUES $valores")) === false) {
                $sClauError = 'TelecoPersonaEx.insertar.prepare';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
                return false;
            } else {
                try {
                    ...
                    $err_txt = $e->errorInfo[2];
                    $this->setErrorTxt($err_txt);
                    $sClauError = 'TelecoPersonaEx.insertar.execute';
                    $_SESSION['oGestorErrores']->addErrorAppLastError($oDblSt, $sClauError, __LINE__, __FILE__);
                    return false;
                }
            }
    ...
    public function DBCarregar($que = null)
    {
        ...
        if (isset($this->iid_item)) {
            if (($oDblSt = $oDbl->query("SELECT * FROM $nom_tabla WHERE id_item=$this->iid_item")) === false) {
                $sClauError = 'TelecoPersonaEx.carregar';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClauError, __LINE__, __FILE__);
                return false;
            }
    ...
    function setAllAttributes(array $aDades)
    {
        ...
        if (array_key_exists('id_desc_teleco', $aDades)) $this->setDesc_teleco($aDades['id_desc_teleco']);
