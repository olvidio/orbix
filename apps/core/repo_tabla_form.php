...
if (!empty($Qobj_pau)) {
    ...
    $sQuery = http_build_query($aQuery);
    $Qgo_to = Hash::link(ConfigGlobal::getWeb() . "/apps/core/repo_tabla_sql.php?$sQuery");
}
...
<script>
    fnjs_grabar = function (formulario) {
        // Comporbar campos: De momento lo quito para cuando trabaje con repositories:
        /*
        var rr = fnjs_comprobar_campos(formulario, '<?= addslashes(get_class($oFicha)) ?>');
        if (rr === 'ok') {
         */
            var url = "apps/core/repo_tabla_update.php";
