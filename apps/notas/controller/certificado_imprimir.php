...
<td align="center"><span class=link onclick='window.open("<?= core\ConfigGlobal::getWeb() ?>/apps/notas/controller/certificado_2_mpdf.php?id_nom=<?= $id_nom ?>&id_tabla=<?= $id_tabla ?><?= $h ?>&PHPSESSID=<?= session_id(); ?>", "sele");' >
<?= _("PDF"); ?></span></td>
</tr></table>
<?php 
if ($Qcara=="A") {
...
while ( $a < count($cAsignaturas)) {
	...
	while (($oAsignatura->getId_nivel() < $row["id_nivel_asig"]) && ($row["id_nivel"] < 2434) ){
		...
		<?php
		$oAsignatura=$cAsignaturas[$a++];
		if ($Qcara=="A" && $oAsignatura->getId_nivel() >= 2200 )	{ continue 2; }
	}
	...
	if ($oAsignatura->getId_nivel() == $row["id_nivel_asig"]) {
		...
		if ($row["id_asignatura"] > 3000 &&  $row["id_asignatura"] < 9000 ) {
			...
			<td class="dato"><?= $row["nota_txt"];?>&nbsp;</td><td></td></tr>
			<?php 
		} else {
			$nombre_asignatura = strtr($oAsignatura->getNombre_asignatura(), $replace);
			?>
