...
fnjs_modificar=function(formulario){
	...
	if (rta==1) {
		$('#mod').val("editar");
  		$(formulario).attr('action',"apps/dossiers/controller/form_3101.php");
  		fnjs_enviar_formulario(formulario,'#ficha_activ');
  	}
}
...
while (list ($clave, $val) = each ($a_ref_perm)) {
	...
	$nom=$val["nom"];
   	if (!empty($permis)) {
		$pagina="apps/dossiers/controller/form_3101.php?que_dl=".core\ConfigGlobal::mi_dele()."&pau=$pau&tabla_p=$tabla_p&id_pau=$id_pau&go_to=$go_to";
