...
fnjs_modificar=function(formulario){
	...
	if (rta==1) {
		$('#mod').val("editar");
  		$(formulario).attr('action',"apps/dossiers/controller/form_1301.php");
...
if (!empty($ref_perm)) { // si es nulo, no tengo permisos de ningún tipo
	...
	while (list ($clave, $val) = each ($ref_perm)) {
		...
		$nom=$val["nom"];
		if (!empty($permis)) {
			$pagina="apps/dossiers/controller/form_1301.php?mod=nuevo&que_dl=".core\ConfigGlobal::mi_dele()."&pau=$pau&db=".core\ConfigGlobal::$db_dl."&id_tipo=$clave&id_pau=$id_pau&go_to=$go_to";
	...
	while (list ($clave, $val) = each ($ref_perm)) {
		...
		$nom=$val["nom"];
		if (!empty($permis)) {
			$pagina="apps/dossiers/controller/form_1301.php?mod=nuevo&pau=$pau&db=".core\ConfigGlobal::$db_dl."&id_tipo=$clave&id_pau=$id_pau&go_to=$go_to";
			echo "<td class=botones><span class=link_inv onclick=\"fnjs_update_div('#ficha_personas','$pagina');\">$nom</span></td>";
		}
	}
	echo "</tr></table></form>";
