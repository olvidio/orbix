	...
	foreach ($cZonas as $oZona) {
		...
		foreach ($cCentrosDl as $oCentroDl) {
			...
			foreach ($cPersonas as $oPersonaNAgd) {
				...
				$sf_ctr=array();
				$sf_cgi=array();
				if (!empty($id_grupo)) { $poblacion=$array_grupos[$id_grupo]; } else { $poblacion=_("otros"); }
				...
				*/
				...
				if ($permiso_sf=="si") {
					...
					if ($sf_ctr[3]==1) { $sf_txt.=", ".sprintf(_("%s centro sf"),$sf_ctr[3]); } elseif ($sf_ctr[3]>1) { $sf_txt.=", ".sprintf(_("%s centros sf"),$sf_ctr[3]); }
					if ($sf_ctr[4]==1) { $sf_txt.=", ".sprintf(_("suplente de %s centro sf"),$sf_ctr[4]); } elseif ($sf_ctr[4]>1) { $sf_txt.=", ".sprintf(_("suplente de %s centros sf"),$sf_ctr[4]); }
					if ($sf_cgi[3]==1) { $sf_txt.=", ".sprintf(_("atiende %s colegio sf"),$sf_cgi[3]); } elseif ($sf_cgi[3]>1) { $sf_txt.=", ".sprintf(_("atiende %s colegios sf"),$sf_cgi[3]); }
					if ($sf_cgi[4]==1) { $sf_txt.=", ".sprintf(_("colabora con %s colegio sf"),$sf_cgi[4]); } elseif ($sf_cgi[4]>1) { $sf_txt.=", ".sprintf(_("colabora con %s colegios sf"),$sf_cgi[4]); }
					$sf_txt=substr($sf_txt,2);
				}
