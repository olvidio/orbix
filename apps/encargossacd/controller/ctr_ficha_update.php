...
case "nuevo": //nuevo
	...
	if (!empty($Qdedic_ctr_m)) { $GesEncargoTipo->insert_horario_ctr($id_enc,'m',$Qdedic_ctr_m,$Qn_sacd); }
	// horario de tarde 1ª hora (t-tarda)
	if (!empty($Qdedic_ctr_t)) { $GesEncargoTipo->insert_horario_ctr($id_enc,'t',$Qdedic_ctr_t,$Qn_sacd); }
	// horario de tarde 2ª hora (v-vespre)
	if (!empty($Qdedic_ctr_v)) { $GesEncargoTipo->insert_horario_ctr($id_enc,'v',$Qdedic_ctr_v,$Qn_sacd); }
...
case "editar": //modificar 
	...
	for($i=0;$i<$Qsacd_num;$i++) {
		if ($i>0) { // sacd colaboradores
			if (!empty($QAid_sacd[$i])) { $GesEncargoTipo->insert_sacd($Qid_enc,$QAid_sacd[$i],5); }
		} else { // sacd titular
			$GesEncargoSacd = new GestorEncargoSacd();
			$aWhere = array();
			$aOperador = array();
