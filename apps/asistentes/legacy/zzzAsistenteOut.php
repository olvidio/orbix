...
use personas\legacy\PersonaOut;
    ...
    public function DBGuardar($quiet = 0)
    {
        ...
        if ($bInsert === false) {
            ...
            if (!is_true($this->btraslado)) {
                $oPersona = Persona::findPersonaEnGlobal($this->iid_nom);
                if (!is_object($oPersona)) {
                    $msg_err = "<br>No encuentro a nadie con id_nom: $this->iid_nom en  " . __FILE__ . ": line " . __LINE__;
                    exit($msg_err);
                }
                $oPersona->DBCarregar();
                // No para los de paso:
