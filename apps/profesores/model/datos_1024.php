...
switch ($padre) {
	case 'datos_sql':
		...
		$oLista=new GestorCongreso();
		$Coleccion=$oLista->getCongresos(array('id_nom'=>$_POST['id_pau']));
		break;
	case 'datos_update':
		// para el update
		if ($_POST['mod'] == 'editar' || $_POST['mod'] == 'eliminar') {
			if (!empty($a_pkey)) $oFicha = new Congreso($a_pkey);
		}
		if ($_POST['mod'] == 'nuevo') {
			$oFicha = new Congreso(array('id_nom'=>$_POST['id_pau']));
		}
		break;
	case 'datos_form':
		// para el form
