...
switch ($Qque) {
	...
	case 'form_pres':
		...
		$oCartaPresentacion = new CartaPresentacion($Qid_ubi);
	...
	case "actualizar":
		...
		// sólo los de la dl.
		$GesCartasPresentacion = new GestorCartaPresentacion();
	...
	case "get_r":
		...
		foreach ($cCentros as $oCentro) {
			$c++;
			$id_ubi = $oCentro->getId_ubi();
			$id_direccion = $oCentro->getId_direccion();
			$nombre_ubi = $oCentro->getNombre_ubi();
			$tipo_ctr = $oCentro->getTipo_ctr();
			$tipo_labor = $oCentro->getTipo_labor();
			$tipo_ubi = $oCentro->getTipo_ubi();
			...
			$oDireccion = new Direccion($id_direccion);
