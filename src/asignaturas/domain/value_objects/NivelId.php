...
final class NivelId
{
    ...
    private function validate(int $value): void
    {
        ...
        if (!($esRangoNormal || $esExcepcion)) {
            throw new \InvalidArgumentException(
                'NivelId must be a 4-digit integer starting with 1, 2 or 3, or be 9998/9999'
            );
        }
    }
    ...
    public function equals(NivelId $other): bool
    {
    ...
    public static function fromString(string $value): self
    {
        if (!ctype_digit($value)) {
            throw new \InvalidArgumentException('NivelId string must be digits');
        }
        // Debe tener exactamente 4 dígitos
        if (strlen($value) !== 4) {
            throw new \InvalidArgumentException('NivelId string must be exactly 4 digits');
        }
