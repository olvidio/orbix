...
/**
 * Escrituras y lecturas sobre `cp_sacd` para una conexión/esquema concretos.
 *
 * A diferencia de {@see PgPersonaSacdRepository} —que fija `oDBC` en el
 * constructor y sirve a la aplicación web— aquí la conexión llega en el
 * {@see CpSacdContexto}, para que la reconciliación multi-esquema pueda usar
 * la conexión de mantenimiento sobre cualquier esquema.
 *
 * El upsert es UPDATE y, si no toca ninguna fila, INSERT. No se usa
 * `ON CONFLICT` a propósito: la clave primaria de `cp_sacd` no es la misma en
 * todas las instalaciones (hay esquemas con `id_schema` en la pkey) y el
 * destino de un ON CONFLICT tiene que coincidir con un índice único existente.
 */
class CpSacdWriter
{
     ...
     */
    public function upsert(CpSacdContexto $contexto, array $fila): bool
    {
        $fila = CpSacdFila::desdeRegistro($fila);
    ...
    public function eliminar(CpSacdContexto $contexto, int $id_nom): bool
    {
        ...
        return $this->eliminarVarios($contexto, [$id_nom]) !== false;
    }
    ...
    public function eliminarVarios(CpSacdContexto $contexto, array $ids): int|false
    {
        ...
        foreach (array_chunk($ids, 500) as $lote) {
            ...
            $sql = "DELETE FROM $tabla WHERE id_nom IN (" . implode(',', $marcadores) . ')';
     ...
     *
     * @return array<int, array<string, mixed>>|false id_nom => fila
     */
    public function filasActuales(CpSacdContexto $contexto): array|false
    {
        $tabla = $contexto->tabla();
        $columnas = implode(',', CpSacdFila::COLUMNAS);
     ...
     *
     * `cp_sacd` se crea una vez por esquema de comun, así que en condiciones
     * normales esto está vacío. Si no lo está, la instalación tiene la copia
     * compartida entre esquemas y la reconciliación no debe borrar esas filas:
     * pertenecen a otra dl.
     *
     * @return list<int>
     */
    public function idNomsDeOtroEsquema(CpSacdContexto $contexto): array
    {
        if ($contexto->id_schema <= 0) {
            return [];
        }
        ...
        $tabla = $contexto->tabla();
        $sql = "SELECT id_nom FROM $tabla WHERE id_schema IS NOT NULL AND id_schema <> :id_schema";
