...
/* -------------------- ENTIDAD --------------------------------------------- */
	...
	public function Guardar(CasaPeriodo $CasaPeriodo): bool
    {
		...
		$aDatos = [];
		$aDatos['id_ubi'] = $CasaPeriodo->getId_ubi();
		$aDatos['sfsv'] = $CasaPeriodo->getSfsv();
		// para las fechas
		$aDatos['f_ini'] = (new ConverterDate('date', $CasaPeriodo->getF_ini()))->toPg();
		$aDatos['f_fin'] = (new ConverterDate('date', $CasaPeriodo->getF_fin()))->toPg();
