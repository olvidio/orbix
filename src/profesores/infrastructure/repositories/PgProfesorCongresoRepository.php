...
/**
 * Clase que adapta la tabla d_congresos a la interfaz del repositorio
 *
 * @package orbix
 * @subpackage model
...
class PgProfesorCongresoRepository extends ClaseRepository implements ProfesorCongresoRepositoryInterface
{
     ...
     * @return array|false Una colección de objetos de tipo ProfesorCongreso
     */
    public function getProfesorCongresos(array $aWhere = [], array $aOperators = []): array|false
    {
        $oDbl = $this->getoDbl();
        $nom_tabla = $this->getNomTabla();
        $ProfesorCongresoSet = new Set();
        ...
        if (($oDblSt = $oDbl->prepare($sQry)) === false) {
            $sClaveError = 'PgProfesorCongresoRepository.listar.prepare';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        if (($oDblSt->execute($aWhere)) === false) {
            $sClaveError = 'PgProfesorCongresoRepository.listar.execute';
    ...
    public function Eliminar(ProfesorCongreso $ProfesorCongreso): bool
    {
        $id_item = $ProfesorCongreso->getId_item();
        $oDbl = $this->getoDbl();
        $nom_tabla = $this->getNomTabla();
        if (($oDbl->exec("DELETE FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorCongresoRepository.eliminar';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        return TRUE;
     ...
     */
    public function Guardar(ProfesorCongreso $ProfesorCongreso): bool
    {
        $id_item = $ProfesorCongreso->getId_item();
        $oDbl = $this->getoDbl();
        $nom_tabla = $this->getNomTabla();
        ...
        if ($bInsert === false) {
            ...
            if (($oDblSt = $oDbl->prepare("UPDATE $nom_tabla SET $update WHERE id_item = $id_item")) === false) {
                $sClaveError = 'PgProfesorCongresoRepository.update.prepare';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
                return false;
            }
            ...
            try {
                ...
                $sClaveError = 'PgProfesorCongresoRepository.update.execute';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDblSt, $sClaveError, __LINE__, __FILE__);
                return false;
            }
        } else {
            ...
            if (($oDblSt = $oDbl->prepare("INSERT INTO $nom_tabla $campos VALUES $valores")) === false) {
                $sClaveError = 'PgProfesorCongresoRepository.insertar.prepare';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
                return false;
            }
            try {
                ...
                $err_txt = $e->errorInfo[2];
                $this->setErrorTxt($err_txt);
                $sClaveError = 'PgProfesorCongresoRepository.insertar.execute';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDblSt, $sClaveError, __LINE__, __FILE__);
                return false;
            }
        }
    ...
    private function isNew(int $id_item): bool
    {
        ...
        if (($oDblSt = $oDbl->query("SELECT * FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorCongresoRepository.isNew';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        if (!$oDblSt->rowCount()) {
    ...
    public function datosById(int $id_item): array|bool
    {
        ...
        $nom_tabla = $this->getNomTabla();
        if (($oDblSt = $oDbl->query("SELECT * FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorCongresoRepository.getDatosById';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
    ...
    {
        $oDbl = $this->getoDbl();
        $sQuery = "select nextval('d_congresos_id_item_seq'::regclass)";
