...
/**
 * Clase que adapta la tabla d_profesor_director a la interfaz del repositorio
 *
 * @package orbix
 * @subpackage model
 ...
 * @created 29/11/2025
 */
class PgProfesorDirectorRepository extends ClaseRepository implements ProfesorDirectorRepositoryInterface
{
    ...
    public function getProfesorDirectores(array $aWhere = [], array $aOperators = []): array|false
    {
        ...
        if (($oDblSt = $oDbl->prepare($sQry)) === false) {
            $sClaveError = 'PgProfesorDirectorRepository.listar.prepare';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        if (($oDblSt->execute($aWhere)) === false) {
            $sClaveError = 'PgProfesorDirectorRepository.listar.execute';
    ...
    public function Eliminar(ProfesorDirector $ProfesorDirector): bool
    {
        $id_item = $ProfesorDirector->getId_item();
        $oDbl = $this->getoDbl();
        $nom_tabla = $this->getNomTabla();
        if (($oDbl->exec("DELETE FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorDirectorRepository.eliminar';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        return TRUE;
     ...
     */
    public function Guardar(ProfesorDirector $ProfesorDirector): bool
    {
        $id_item = $ProfesorDirector->getId_item();
        $oDbl = $this->getoDbl();
        $nom_tabla = $this->getNomTabla();
        ...
        if ($bInsert === false) {
            ...
            if (($oDblSt = $oDbl->prepare("UPDATE $nom_tabla SET $update WHERE id_item = $id_item")) === false) {
                $sClaveError = 'PgProfesorDirectorRepository.update.prepare';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
                return false;
            }
            ...
            try {
                ...
                $sClaveError = 'PgProfesorDirectorRepository.update.execute';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDblSt, $sClaveError, __LINE__, __FILE__);
                return false;
            }
        } else {
            // INSERT
            $aDatos['id_item'] = $ProfesorDirector->getId_item();
            $campos = "(id_item,id_nom,id_departamento,escrito_nombramiento,f_nombramiento,escrito_cese,f_cese)";
            $valores = "(:id_item,:id_nom,:id_departamento,:escrito_nombramiento,:f_nombramiento,:escrito_cese,:f_cese)";
            if (($oDblSt = $oDbl->prepare("INSERT INTO $nom_tabla $campos VALUES $valores")) === false) {
                $sClaveError = 'PgProfesorDirectorRepository.insertar.prepare';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
                return false;
            }
            try {
                ...
                $err_txt = $e->errorInfo[2];
                $this->setErrorTxt($err_txt);
                $sClaveError = 'PgProfesorDirectorRepository.insertar.execute';
                $_SESSION['oGestorErrores']->addErrorAppLastError($oDblSt, $sClaveError, __LINE__, __FILE__);
                return false;
            }
        }
    ...
    private function isNew(int $id_item): bool
    {
        ...
        if (($oDblSt = $oDbl->query("SELECT * FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorDirectorRepository.isNew';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
        if (!$oDblSt->rowCount()) {
    ...
    public function datosById(int $id_item): array|bool
    {
        ...
        $nom_tabla = $this->getNomTabla();
        if (($oDblSt = $oDbl->query("SELECT * FROM $nom_tabla WHERE id_item = $id_item")) === false) {
            $sClaveError = 'PgProfesorDirectorRepository.getDatosById';
            $_SESSION['oGestorErrores']->addErrorAppLastError($oDbl, $sClaveError, __LINE__, __FILE__);
            return false;
        }
     ...
     */
    public function findById(int $id_item): ?ProfesorDirector
    {
    ...
    {
        $oDbl = $this->getoDbl();
        $sQuery = "select nextval('d_profesor_director_id_item_seq'::regclass)";
