 ...
 *
 * Uso:
 *   php tools/fix/migrar_inc_formato_partido.php comun
 *   php tools/fix/migrar_inc_formato_partido.php comun sv
 *   php tools/fix/migrar_inc_formato_partido.php --dir=/home/dani/docker_images/orbix/web/html/conf comun
 *   ORBIX_DIR_PWD=/ruta/al/conf php tools/fix/migrar_inc_formato_partido.php comun
 *
...
function migrar_parse_argv(array $argv): array
{
    ...
    foreach (array_slice($argv, 1) as $arg) {
        ...
        if ($arg === '--help' || $arg === '-h') {
            fwrite(STDOUT, "Uso: php tools/fix/migrar_inc_formato_partido.php [--dir=/ruta/conf] comun [sv] …\n");
            exit(0);
        }
        $bases[] = $arg;
    }
...
function migrar_resolver_dir_conf(string $dirCli): string
{
    ...
    fwrite(STDERR, "  php tools/fix/migrar_inc_formato_partido.php --dir=/home/dani/docker_images/orbix/web/html/conf comun\n");
    fwrite(STDERR, "  ORBIX_DIR_PWD=/ruta/conf php tools/fix/migrar_inc_formato_partido.php comun\n");
    exit(1);
}
