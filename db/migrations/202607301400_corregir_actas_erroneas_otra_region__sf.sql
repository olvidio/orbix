...
--
-- Orden: después de 281100. Serie sf.
...
INSERT INTO publicf._mig_mapa_prefijo_acta_esquema (pref, esquema_base) VALUES
    ('a', 'Euc-crEuc'),
    ('aes', 'Aes-crAes'),
    ('arg', 'Pla-crPla'),
    ('aso', 'Aes-crAes'),
...
CREATE TABLE IF NOT EXISTS publicf._mig_actas_erroneas (
    acta_old      text PRIMARY KEY,
    acta_new      text,
    esquema_token text
);

TRUNCATE publicf._mig_actas_erroneas;

-- @orbix_import_csv: db/migrations/data/actas_erroneas.csv
-- @orbix_import_into: publicf._mig_actas_erroneas(acta_old, acta_new, esquema_token)
-- @orbix_import_here

DO $$
BEGIN
    IF (SELECT count(*) FROM publicf._mig_actas_erroneas) < 1 THEN
        RAISE EXCEPTION
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            ...
            SET acta = m.acta_new
            FROM publicf._mig_actas_erroneas m
    ...
    LOOP
        FOR r IN EXECUTE format(
            ...
            JOIN publicf._mig_actas_erroneas m
    ...
    PERFORM public.migracion_aviso(format(
        'actas erroneas sf: reescritas=%s plan_traslado=%s insertadas=%s borradas_origen=%s sin_destino=%s omitidas_sin_dl=%s',
        n_upd_total, n_plan, n_ins_total, n_del_total, n_sin_dest, n_omit_dl
    ));
END $$;

DROP TABLE IF EXISTS publicf._mig_actas_erroneas;
DROP TABLE IF EXISTS publicf._mig_mapa_prefijo_acta_esquema;

SELECT public.migracion_aviso('actas erroneas sf: snapshots temporales eliminados');
