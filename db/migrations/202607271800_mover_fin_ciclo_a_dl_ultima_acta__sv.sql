...
-- Serie sv. El snapshot temporal se elimina al final.
...
BEGIN
    ...
    PERFORM public.migracion_aviso(format(
        'mover fin ciclo (9998/9999) sv: planificadas=%s insertadas=%s borradas_origen=%s ya_ok=%s sin_destino=%s omitidas_sin_dl=%s',
...
SELECT public.migracion_aviso('snapshot _mig_mapa_prefijo_acta_esquema eliminado (sv, 271800)');
