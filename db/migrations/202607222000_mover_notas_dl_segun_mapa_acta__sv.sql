...
-- (prefijo del acta). Ej.: «dlp …» en H-dlbv → H-dlpv.
--
...
BEGIN
    ...
    PERFORM public.migracion_aviso(format(
        'mover e_notas_dl mal ubicadas sv: insertadas=%s borradas_origen=%s omitidas_sin_destino=%s',
...
SELECT public.migracion_aviso('snapshot _mig_mapa_prefijo_acta_esquema eliminado (sv)');
