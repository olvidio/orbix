-- Equivalente sf de 202604110000_stgr_to_nivel_stgr__sv.sql (sin réplica; esquemas *f / publicf).
...
BEGIN
    IF migracion_columna_existe('global', 'personas', 'stgr') THEN
        ...
        FROM publicf.xa_nivel_stgr_tmp x
...
DO $$
BEGIN
    IF migracion_columna_existe('publicf', 'p_de_paso', 'stgr') THEN
        UPDATE publicf.p_de_paso SET stgr = 'c1' WHERE stgr = 'c';
        ...
        PERFORM migracion_rename_columna('publicf', 'p_de_paso', 'stgr', 'nivel_stgr');
    END IF;

    IF migracion_columna_existe('publicf', 'p_de_paso', 'nivel_stgr')
       AND NOT EXISTS (
           SELECT 1
           FROM information_schema.columns
           WHERE table_schema = 'publicf'
             AND table_name = 'p_de_paso'
             AND column_name = 'nivel_stgr'
             AND data_type = 'integer'
       )
    THEN
        ALTER TABLE publicf.p_de_paso
            ALTER COLUMN nivel_stgr TYPE integer USING nivel_stgr::integer;
    ELSIF migracion_columna_existe('publicf', 'p_de_paso', 'nivel_stgr') THEN
        PERFORM migracion_aviso('publicf.p_de_paso.nivel_stgr ya es integer (omitido)');
    END IF;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            'UPDATE %I.%I p SET stgr = x.nivel_stgr::text FROM publicf.xa_nivel_stgr_tmp x WHERE p.stgr = x.desc_breve',
            table_record.schema_name,
            table_record.table_name
        );
