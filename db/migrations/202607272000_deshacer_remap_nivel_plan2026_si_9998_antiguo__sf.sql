...
-- Corte: el mismo que 152200 (2026-09-30). Serie sf.
...
BEGIN
    ...
    -- ------------------------------------------------------------------
    DELETE FROM publicf.e_notas AS n
    ...
    WHERE n.id_asignatura = 3411
      ...
      AND EXISTS (
          SELECT 1
          FROM publicf.e_notas AS heb
          WHERE heb.id_nom = n.id_nom
            AND heb.id_asignatura = 2112
      )
      AND EXISTS (
          SELECT 1
          FROM publicf.e_notas AS gre
    ...
    PERFORM public.migracion_aviso(format(
        'deshacer conv+remap plan 2026 sf: del_pc_conv=%s del_op_3411=%s remap_pc=%s latin4=%s latin4_int=%s latin3=%s op_bienio=%s',
