...
final class ActividadtarifasPayload
{
    ...
    {
        return [
            'id_tarifa' => PayloadCoercion::string($payload['id_tarifa'] ?? 'nuevo'),
            'es_nuevo' => ($payload['es_nuevo'] ?? true) === true,
            'letra' => PayloadCoercion::string($payload['letra'] ?? ''),
            'modo' => PayloadCoercion::int($payload['modo'] ?? 0),
            'observ' => PayloadCoercion::string($payload['observ'] ?? ''),
            'opciones_modo' => NotasFormSupport::desplegableOpciones($payload['opciones_modo'] ?? []),
            'id_item' => PayloadCoercion::string($payload['id_item'] ?? ''),
            'id_tipo_activ' => PayloadCoercion::int($payload['id_tipo_activ'] ?? 0),
            'id_tarifa_sel' => PayloadCoercion::int($payload['id_tarifa_sel'] ?? 0),
            'isfsv' => PayloadCoercion::int($payload['isfsv'] ?? 0),
            'nom_tipo_activ' => PayloadCoercion::string($payload['nom_tipo_activ'] ?? ''),
            ...
            'cantidad' => PayloadCoercion::string($payload['cantidad'] ?? ''),
            'opciones_serie' => NotasFormSupport::desplegableOpciones($payload['opciones_serie'] ?? []),
            'id_serie_sel' => PayloadCoercion::int($payload['id_serie_sel'] ?? 1),
            'token_update' => PayloadCoercion::string($payload['token_update'] ?? ''),
            'token_eliminar' => PayloadCoercion::string($payload['token_eliminar'] ?? ''),
            ...
            'any_actual' => PayloadCoercion::int($payload['any_actual'] ?? 0),
            'token_copiar' => PayloadCoercion::string($payload['token_copiar'] ?? ''),
        ];
    }
