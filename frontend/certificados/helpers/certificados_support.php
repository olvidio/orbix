 ...
 */
function certificados_parse_id_nom_from_sel_value(mixed $raw): int
{
    if (!is_scalar($raw)) {
        return 0;
    }
...
function certificados_id_nom_from_sel_post(): int
{
    $a_sel_raw = filter_input(INPUT_POST, 'sel', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY);
    if (is_array($a_sel_raw) && $a_sel_raw !== []) {
        $idNom = certificados_parse_id_nom_from_sel_value($a_sel_raw[0]);
        if ($idNom > 0) {
            return $idNom;
        }
    }

    $idNom = certificados_parse_id_nom_from_sel_value($_POST['id_sel'] ?? null);
    if ($idNom > 0) {
        return $idNom;
    }
