...
final class UsuarioAvisosPrefFormRender
{
    ...
    public static function enrich(array $result): array
    {
        ...
        $result['url_guardar_objeto'] = $abs(PayloadCoercion::string($paths['cambio_usuario_objeto_pref_guardar'] ?? ''));
        $result['url_guardar_propiedades'] = $abs(PayloadCoercion::string($paths['cambio_usuario_propiedad_pref_guardar_todas'] ?? ''));
        $result['url_preview_cond'] = $abs(PayloadCoercion::string($paths['cambio_usuario_propiedad_pref_preview'] ?? ''));
        $result['url_get_propiedades'] = $abs(PayloadCoercion::string($paths['usuario_avisos_pref_propiedades'] ?? ''));
        $result['url_get_condicion'] = $abs(PayloadCoercion::string($paths['usuario_avisos_pref_condicion'] ?? ''));
        $result['url_get_fases'] = $abs(PayloadCoercion::string($paths['usuario_avisos_pref_fases'] ?? ''));
        ...
        $oHash->setCamposNo(PayloadCoercion::string($hm['campos_no'] ?? ''));
        $chk = PayloadCoercion::string($hm['campos_chk'] ?? '');
        if ($chk !== '') {
        ...
        $oHashFases->setUrl($abs(PayloadCoercion::string($hf['path'] ?? '')));
        $oHashFases->setCamposForm(PayloadCoercion::string($hf['campos_form'] ?? ''));
        ...
        $oHashProp->setUrl($abs(PayloadCoercion::string($hp['path'] ?? '')));
        $oHashProp->setCamposForm(PayloadCoercion::string($hp['campos_form'] ?? ''));
        ...
        $oHashMod->setUrl($abs(PayloadCoercion::string($hm2['path'] ?? '')));
        $oHashMod->setCamposForm(PayloadCoercion::string($hm2['campos_form'] ?? ''));
        $result['h_mod'] = $oHashMod->linkSinValParams();

        $perm_jefe = (bool)($result['perm_jefe'] ?? false);
        $id_tipo_res = PayloadCoercion::string($result['id_tipo_activ'] ?? '');
        ...
        if ($id_tipo_res !== '') {
            ...
            $oTipoActiv->setSfsvText(PayloadCoercion::string($result['sfsv_text'] ?? ''));
