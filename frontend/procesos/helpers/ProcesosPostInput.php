...
public static function selTokensFromPost(): array
{
    ...
    if (!is_array($a_sel_raw) || $a_sel_raw === []) {
        return [
            ...
            'id_tipo_activ_txt' => ActividadTipoIdTxt::canonicalize(self::postString('id_tipo_activ_txt')),
    ...
    return [
        ...
        'id_item' => \frontend\shared\helpers\PayloadCoercion::string($parts[1] ?? ''),
        'id_usuario' => is_numeric($id0) ? (int) $id0 : 0,
        'id_tipo_activ_txt' => ActividadTipoIdTxt::canonicalize(
            \frontend\shared\helpers\PayloadCoercion::string($parts[2] ?? '')
        ),
        'dl_propia' => \frontend\shared\helpers\PayloadCoercion::string($parts[3] ?? ''),
    ];
}
