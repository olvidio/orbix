...
foreach ($cTipoDossier as $oTipoDossier) {
    ...
    $a_filas[$i]['href_ver'] = Hash::link(ConfigGlobal::getWeb() . '/frontend/dossiers/controller/dossiers_ver.php?' . http_build_query(array('pau' => $pau, 'id_pau' => $id_pau, 'obj_pau' => $Qobj_pau, 'id_dossier' => $id_dossier, 'permiso' => $perm_a, 'depende' => $depende_modificar)));
    $a_filas[$i]['href_abrir'] = Hash::link(ConfigGlobal::getWeb() . '/frontend/dossiers/controller/dossier_abrir.php?' . http_build_query(array('pau' => $pau, 'id_pau' => $id_pau, 'obj_pau' => $Qobj_pau, 'id_dossier' => $id_dossier, 'tabla_to' => $tabla_to, 'permiso' => $perm_a)));
