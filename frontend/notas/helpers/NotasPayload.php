...
final class NotasPayload
{
    ...
    public static function actaVerFormFromPayload(array $payload): array
    {
        ...
        return [
            ...
            'acta_sigla' => PayloadCoercion::string($payload['acta_sigla'] ?? ''),
            'acta_new_num' => PayloadCoercion::string($payload['acta_new_num'] ?? ''),
        ...
        if (is_array($aAsigRaw)) {
            foreach ($aAsigRaw as $key => $value) {
                if (is_int($key)) {
                    ...
                    $aAsignaturas[PayloadCoercion::int($key)] = PayloadCoercion::string($value);
                }
            }
        }
    ...
    public static function personaFormFromPayload(array $payload): array
    {
        ...
        return [
            ...
            'acta_sigla_sel' => PayloadCoercion::string($payload['acta_sigla_sel'] ?? ''),
            'acta_num' => PayloadCoercion::string($payload['acta_num'] ?? ''),
            'acta_cert_dl_sel' => PayloadCoercion::string($payload['acta_cert_dl_sel'] ?? ''),
            'acta_cert_num' => PayloadCoercion::string($payload['acta_cert_num'] ?? ''),
            'mi_sigla' => PayloadCoercion::string($payload['mi_sigla'] ?? ''),
            'dl_sin_esquema' => NotasFormSupport::desplegableOpciones($payload['dl_sin_esquema'] ?? []),
            'opciones_certificado_dl' => NotasFormSupport::desplegableOpciones($payload['opciones_certificado_dl'] ?? []),
