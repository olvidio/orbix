...
final class CartasPresentacionShellRender
{
    ...
    public static function enrich(array $payload): array
    {
        ...
        $oHashLista->setCamposForm((string)($hl['campos_form'] ?? ''));
        $oHashLista->setCamposNo((string)($hl['campos_no'] ?? ''));
        $hash_lista_html = $oHashLista->getCamposHtml();

        $url_form = $abs((string)($paths['form'] ?? ''));
        $hf = isset($payload['hash_form']) && is_array($payload['hash_form']) ? $payload['hash_form'] : [];
        $oHashForm = new HashFront();
        $oHashForm->setUrl($url_form);
        $oHashForm->setCamposForm((string)($hf['campos_form'] ?? ''));
        $h_form = $oHashForm->linkSinVal();

        $url_poblaciones = $abs((string)($paths['poblaciones'] ?? ''));
        ...
        $oHashPob->setCamposForm((string)($hp['campos_form'] ?? ''));
        $h_poblaciones = $oHashPob->linkSinValParams();

        $url_eliminar = $abs((string)($paths['eliminar'] ?? ''));
        ...
        $oHashEliminar->setCamposForm((string)($he['campos_form'] ?? ''));
        ...
        $payload['url_update'] = $abs((string)($paths['update'] ?? ''));
