...
class ods {
	...
	function getSettings($settings_propio) {
		if (!empty($settings_propio)) {
			if ($rta=file_get_contents($setting_propio)) {
				return $rta; 
			}
		}
		/*	return '<?xml version="1.0" encoding="UTF-8"?>
...
function saveOds($obj,$file,$txt_html,$conv_style) {
...
// FIN de  Cabecera global de URL de controlador ********************************
	...
	$uid = uniqid();
	mkdir($tmp.'/'.$uid);
	file_put_contents($tmp.'/'.$uid.'/content.xml',$txt_html); // si uso xsltproc el texto ya tiene las cabeceras.
	...
	file_put_contents($tmp.'/'.$uid.'/styles.xml',$obj->getStyle($estilo_propio));
	file_put_contents($tmp.'/'.$uid.'/settings.xml',$obj->getSettings($settings_propio));
